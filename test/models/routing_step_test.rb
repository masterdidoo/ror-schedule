...
class RoutingStepTest < ActiveSupport::TestCase
  ...
  private
  def test_same_addresses(step)
    list = step.load_orders.map{|m| m.origin_address}.concat step.unload_orders.map{|m| m.destination_address}
